<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\ProductoPaquete;

/**
 * ProductoPaqueteSearch represents the model behind the search form about `app\models\ProductoPaquete`.
 */
class ProductoPaqueteSearch extends ProductoPaquete
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id', 'producto_id', 'paquete_id', 'cantidad_productos'], 'integer'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = ProductoPaquete::find();

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        $query->andFilterWhere([
            'id' => $this->id,
            'producto_id' => $this->producto_id,
            'paquete_id' => $this->paquete_id,
            'cantidad_productos' => $this->cantidad_productos,
        ]);

        return $dataProvider;
    }
}
